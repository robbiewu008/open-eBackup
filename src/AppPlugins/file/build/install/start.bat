...
:main
    ...
    if %errorlevel% == 1 (
        call :log_echo "INFO" "%PluginName% start failed"
        exit /b 1
    )
    ...
    start %DATA_BACKUP_AGENT_HOME%\DataBackup\ProtectClient\Plugins\%PluginName%\bin\FilePlugin.exe %LOG_PATH% %PORT_BEGIN% %PORT_END% %AGENT_IP% %AGENT_PORT%
