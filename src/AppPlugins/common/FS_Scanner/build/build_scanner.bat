...
if "%Module_Branch%" == "" (
    set Module_Branch=master_OceanProtect_DataBackup
)
...
git-bash.exe -c "cd FS_Scanner/src;find -name *.cpp | xargs unix2dos; find -name *.h | xargs unix2dos;"
git-bash.exe -c "cd Module/src;find -name *.cpp | xargs unix2dos; find -name *.h | xargs unix2dos;"

set SOLUTION=%SCANNER_PATH%Windows_Compile\vsprj\build\Scanner.sln
...
COPY /Y %SCANNER_PATH%Windows_Compile\vsprj\bin\bin\*.lib      %SCANNER_PATH%\FS_Scanner\libs
