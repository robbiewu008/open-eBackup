...
if(TARGET safe_cmplib)
    ...
    if ("${CMAKE_HOST_SYSTEM_NAME}" STREQUAL "AIX")
        target_compile_options(safe_cmplib INTERFACE -fPIC -maix64 -pthread -pipe -shared)
        target_link_options(safe_cmplib INTERFACE -maix64 -pthread)
    elseif ("${CMAKE_HOST_SYSTEM_NAME}" STREQUAL "SunOS")
        target_compile_options(safe_cmplib INTERFACE -fPIC -pipe -pthread)
        target_link_options(safe_cmplib INTERFACE -Wl,-z,now -pthread)
    else()
        target_compile_options(safe_cmplib INTERFACE -fPIC -pipe -fstack-protector-strong -O)
...
if(TARGET safe_cmpexec)
    ...
    if ("${CMAKE_HOST_SYSTEM_NAME}" STREQUAL "SunOS")
        target_compile_options(safe_cmpexec INTERFACE -fPIC -pipe -pthread)
        target_link_options(safe_cmpexec INTERFACE -fpie -Wl,-z,now -pipe -pthread)
    else()
...
if(CMAKE_BUILD_TYPE STREQUAL "Release")
    ...
    if ("${CMAKE_HOST_SYSTEM_NAME}" STREQUAL "SunOS")
        target_link_options(safe_cmplib INTERFACE  "-Wl,-z,now" "-fstack-check" "-D_FORTIFY_SOURCE=2 -O2" "-flto" "-s")
    else()
