...
function copy_files() {
    ...
    cp -rf Infrastructure_OM/ci/build/* ${G_BASE_DIR}/component/INF_CI
 ...
 #   cp -rf ProtectManager/component/PM_API_Gateway/IngressRoute/*   ${G_BASE_DIR}/component/PM_CI/build/helm/protect-manager/templates/
    ...
    find "${G_BASE_DIR}/component" -maxdepth 3 -type d -iregex '.*helm/.*' -exec cp -rf "{}" "${G_BASE_DIR}/build/helm/components" \;
...
function build_open_helm() {
    ...
    if [ "${BUILD_MODULE}" == "system_pm" ] ; then
        ...
        find . -maxdepth 1 -type d ! -name 'infrastructure' ! -name 'protect-manager' ! -name '.' -exec rm -rf {} +
        for h in $(ls "${G_BASE_DIR}/build/helm/components"); do
