...
function build_image() {
    ...
    docker build --rm -t $L_TAG -f "${BASE_PATH}/build/dockerfiles/$L_MS_NAME.dockerfile" "${BASE_PATH}/tmp/${MS_DIR}/mstmp/"
    if [ $? -ne 0 ]; then
    ...
    echo "Upload images to harbor: harbors.inhuawei.com/${harbor_project}"
    if [ 'OceanCyber' == "${BUILD_PKG_TYPE}" ] && [ 'PM_System_Base_Service' == "${L_MS_NAME}" ]; then
        docker tag ${L_TAG} harbors.inhuawei.com/${harbor_project}/${code_branch}/${tag_image}/${PRODUCT_IMAGE_PATH}/${L_TAG}
        docker push harbors.inhuawei.com/${harbor_project}/${code_branch}/${tag_image}/${PRODUCT_IMAGE_PATH}/${L_TAG}
    else
...
function build_all_image() {
    ...
    do
        ...
        build_image $MS_NAME
        if [ $? -ne 0 ];then
            echo "docker iamge build failed"
        fi
     done
}
