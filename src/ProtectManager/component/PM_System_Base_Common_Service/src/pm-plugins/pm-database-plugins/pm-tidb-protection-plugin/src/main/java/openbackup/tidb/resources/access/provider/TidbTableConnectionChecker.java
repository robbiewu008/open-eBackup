...
@Slf4j
@Component
public class TidbTableConnectionChecker extends UnifiedResourceConnectionChecker {
    /**
     * tidb service
     */
     ...
     */
    public TidbTableConnectionChecker(ProtectedEnvironmentRetrievalsService environmentRetrievalsService,
    ...
    public boolean applicable(ProtectedResource object) {
        return Objects.nonNull(object) && ResourceSubTypeEnum.TIDB_TABLE.getType().equals(object.getSubType());
    }
    ...
    public Map<ProtectedResource, List<ProtectedEnvironment>> collectConnectableResources(
        ...
        log.info("TidbTableConnectionChecker,collectConnectableResources,environment.uuid: {}", environment.getUuid());
        ProtectedResource databaseResource = tidbService.getResourceByCondition(environment.getParentUuid());
        ...
        clusterResource.getExtendInfo()
            .put(TidbConstants.DATABASE_NAME, databaseResource.getExtendInfo().get(TidbConstants.DATABASE_NAME));
        clusterResource.getExtendInfo()
            .put(TidbConstants.TABLE_NAME_LIST, environment.getExtendInfo().get(TidbConstants.TABLE_NAME));
        clusterResource.getExtendInfo().put(TidbConstants.ACTION_TYPE, TidbConstants.CHECK_TABLE);
