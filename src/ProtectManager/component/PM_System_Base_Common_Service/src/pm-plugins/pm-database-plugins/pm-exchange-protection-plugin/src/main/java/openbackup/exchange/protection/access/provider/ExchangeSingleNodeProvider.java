...
@Slf4j
@Component
public class ExchangeSingleNodeProvider extends DatabaseEnvironmentProvider {
     ...
     */
    public ExchangeSingleNodeProvider(ProviderManager providerManager, PluginConfigManager pluginConfigManager,
    ...
    public boolean applicable(String resourceSubType) {
        return ResourceSubTypeEnum.EXCHANGE_SINGLE_NODE.getType().equals(resourceSubType);
    }

    @Override
    public void register(ProtectedEnvironment environment) {
        log.info("Start to check exchange single node:{}", environment.getName());
        ...
        if (StringUtils.isNotEmpty(environment.getUuid())) {
            checkUpdateResourceInfo(environment);
        }
        // 校验任务并发数参数
        ...
        exchangeService.queryClusterInfo(environment, null);
    ...
    private String buildUniqueUuid(Set<String> agentsUuids) {
        ...
        /**/
        String envIdentity = ResourceSubTypeEnum.EXCHANGE_SINGLE_NODE.getType() + uuidTag;
    ...
    private void checkUpdateResourceInfo(ProtectedEnvironment environment) {
        // 不允许修改接入类型
    ...
    public List<ProtectedResource> scan(ProtectedEnvironment environment) {
        ...
        environment.getDependencies().get(ExchangeConstant.EXCHANGE_AGENTS).forEach(agentEnvironment -> {
            ...
            do {
                response.set(agentUnifiedService.getDetailPageList(ResourceSubTypeEnum.EXCHANGE_SINGLE_NODE.getType(),
    ...
    private ListResourceV2Req generateListResourceV2Req(int page, int size, ProtectedEnvironment environment,
        ...
        environment.setSubType(ResourceSubTypeEnum.EXCHANGE_SINGLE_NODE.getType());
        ...
        agentResource.setSubType(ResourceSubTypeEnum.EXCHANGE_SINGLE_NODE.getType());
