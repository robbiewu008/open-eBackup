...
class ExchangeDagRestoreInterceptorTest {
    @Mock
    ...
    private ExchangeDagRestoreInterceptor exchangeRestoreInterceptorTest;

    @BeforeEach
    void setUp() {
        exchangeRestoreInterceptorTest = new ExchangeDagRestoreInterceptor(mockExchangeService, copyRestApi, resourceService);
    }
    ...
    public void restore_getLock_resources() {
        ...
        protectedResource.setUuid("test_uuid");
        query.setRecords(Collections.singletonList(protectedResource));
        ...
        Assert.assertEquals(lockResources.get(0).getId(), restoreTask.getTargetObject().getUuid());
    }
