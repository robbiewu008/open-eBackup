...
class ExchangeGroupProviderTest {
    ...
    private ExchangeGroupProvider exchangeGroupProviderUnderTest;

    @BeforeEach
    void setUp() {
        exchangeGroupProviderUnderTest = new ExchangeGroupProvider(mockProviderManager, mockPluginConfigManager,
    ...
    public void should_throw_LegoCheckedException_if_params_has_same_instance() {
        final ProtectedEnvironment environment = getSameAgentEnvironment();
        System.out.println(JsonUtil.json(environment));
        ProtectedEnvironment agentEnvironment = getOldEnvironment();
        agentEnvironment.setEndpoint("8.8.8.8");
        agentEnvironment.setPort(666);
        ...
        uuidAndPortSet.add("2615222a-1993-4f84-8be5-");
        when(mockExchangeService.getExistingUuid(any())).thenReturn(uuidAndPortSet);
    ...
    /**
     * 用例场景：注册集群时，下发参数中存在包含已经注册的实例
     * 前置条件：下发参数中存在包含已经注册的实例
     * 检查点: 报错
     */
    @Test
    public void should_throw_LegoCheckedException_if_params_has_already_registered_instance() {
        final ProtectedEnvironment environment = getEnvironment();
        System.out.println(JsonUtil.json(environment));
        ProtectedEnvironment agentEnvironment = getOldEnvironment();
        agentEnvironment.setEndpoint("8.8.8.8");
        ...
        // Run the test
        Set<String> uuidAndPortSet =  new HashSet<>();
        uuidAndPortSet.add("2615222a-1993-4f84-8be5-7c74dc335a6d");
        when(mockExchangeService.getExistingUuid(any())).thenReturn(uuidAndPortSet);
        LegoCheckedException legoCheckedException = Assert.assertThrows(LegoCheckedException.class,
            () -> exchangeGroupProviderUnderTest.register(environment));
        Assert.assertEquals(ExchangeConstant.CLUSTER_NODE_IS_REGISTERED, legoCheckedException.getErrorCode());
    }
    ...
    private ProtectedEnvironment getSameAgentEnvironment() {
        String json = "{\"name\":\"Server01\",\"type\":\"Database\",\"subType\":\"Exchange-group\",\"extendInfo\":{\"linkStatus\":\"0\",\"agentUuid\":\"2615222a-1993-4f84-8be5-7c74dc335a6d\",\"isGroup\":\"0\"},\"auth\":{\"authType\":2,\"authKey\":\"extest\\\\Administrator\",\"authPwd\":\"xxxxx\"},\"dependencies\":{\"agents\":[{\"uuid\":\"2615222a-1993-4f84-8be5-7c74dc335a6d\",\"inGroup\":false}，{\"uuid\":\"2615222a-1993-4f84-8be5-7c74dc335a6d\",\"inGroup\":false}]},\"scanInterval\":3600,\"inGroup\":false}";
        ProtectedEnvironment read = JsonUtil.read(json, ProtectedEnvironment.class);
        return read;
    }
}
