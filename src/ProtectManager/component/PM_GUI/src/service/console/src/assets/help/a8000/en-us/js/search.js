...
function beforeSearch() {
  ...
  titleOrContext =
    searchOptionSelected.options[searchOptionSelected.selectedIndex].value;
...
function binarySearch(slice, matchTokensMap) {
  var result = new Map();
  var ref, paramto, tokens, tokenStr, tokensBegin, tokensEnd, mid;
...
function analysis(keyword, matchTokensMap) {
  ...
  for (var i = 0, length = keywordArr.length; i < length; i++) {
    ...
    if (isDigitalOrChar(char)) {
      ...
      var flag =
        i + 1 >= keywordArr.length || !isDigitalOrChar(keywordArr[i + 1]);
      if (flag) {
...
function getWordRec(keyword) {
  var paramto, tokens, matchTokens;
