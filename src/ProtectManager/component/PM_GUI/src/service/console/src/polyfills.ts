...
// import "eligrey-classlist-js-polyfill"; // Run `npm install --save classlist.js`.
      ...
      return {
        ...
        add(name: string) {
          if (!this.contains(name)) {
            const names = _this.className.baseVal.split(' ').filter(a => !!a);
            names.push(name);
        ...
        remove(name: string) {
          if (this.contains(name)) {
            const names = _this.className.baseVal
              .split(' ')
              .filter(a => !!a && a !== name);
            _this.setAttribute('class', names.join(' '));
          }
