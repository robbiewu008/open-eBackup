...
RUN pip3 install --no-index --find-links=lib/ --no-cache-dir -r public_cbb/requirements.txt && python3 setup.py install && \
    sed -i 's/v = connection.execute("select version()").scalar()/v = \"PostgreSQL 15.2\"/g' \
    /usr/local/lib/python3.9/site-packages/sqlalchemy/dialects/postgresql/base.py && rm -rf /public_cbb && \
