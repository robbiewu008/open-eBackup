...
sed -i 's/num.partitions.*$/num.partitions=4/g' ${KAFKA_CFG_IN_NAS}
...
sed -i '$a\auto.leader.rebalance.enable=false' ${KAFKA_CFG_IN_NAS}
check_result "$?" "sed auto.leader.rebalance.enable for ${KAFKA_CFG_IN_NAS}"
  ...
  if [[ -n "${master_ip}" ]] && [[ -n "${standby_ip}" ]] && [[ -n "${slave_ip}" ]];then
    ...
    if [[ ${NODE_NAME} == ${MASTER_NODE} ]];then
      ...
      if [[ "${zk_cluster}" == "false" ]]; then
        rm -rf ${DATA_PATH}/meta.properties
      fi
    fi
  else
