...
if [ ! -d ${LOG_PATH} ]; then
  mkdir -p ${LOG_PATH}
  chown -h 99:nobody /opt/OceanProtect/logs/${NODE_NAME}
  chown -h 99:nobody /opt/OceanProtect/logs/${NODE_NAME}/infrastructure
  chmod 750 /opt/OceanProtect/logs/${NODE_NAME}
  chmod 750 /opt/OceanProtect/logs/${NODE_NAME}/infrastructure
  chown -h GaussDB:dbgrp ${LOG_PATH}
  chmod 750 ${LOG_PATH}
fi
...
function log_error() {
  ...
  echo ${err_msg}
  echo ${err_msg} | sudo -u ${GAUSS_USER} tee -a ${LOG_FILE} >/dev/null
}
...
function log_info() {
  ...
  echo ${msg}
  echo ${msg} | sudo -u ${GAUSS_USER} tee -a ${LOG_FILE} >/dev/null
}
