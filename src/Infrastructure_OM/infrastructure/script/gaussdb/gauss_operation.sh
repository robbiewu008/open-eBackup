...
function enter_database()
{
    ...
    # 使用交互式输入
    expect -c "
    spawn su - ${GAUSS_OP_USER} -s /bin/bash -c \"export LD_LIBRARY_PATH=/usr/local/gaussdb/app/lib::/usr/local/app/lib; /usr/local/gaussdb/app/bin/gsql -U ${gauss_usr} -d ${database} -h ${gauss_ip} -p ${gauss_port}\"
    expect \"Password for user gaussdbremote:\"
    send -- \"${gaussdb_pwd}\r\"
    interact
    "
