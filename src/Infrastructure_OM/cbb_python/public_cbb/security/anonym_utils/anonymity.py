...
class Anonymity:
    ...
    """
    sensitive_words = [
        "%pass%", "%pwd%", "%key%", "%crypto%", "%session%", "%token%", "%fingerprint%", "%auth%",
    ...
    def process_ext(cls, data, is_affect_source_data: bool, add_sensitive_list: list):
        ...
        try:
            ...
            return cls._process_data(data_log, all_sensitive_list)
        except Exception as e:
            logger.error(f'Anonymize failed, e={str(e)}')
            return data
    ...
    def _process_json_string(cls, json_str: str, sensitive_word_list: list):
        ...
        try:
            ...
            if not result:
                return json_str
            logger.info(f'Not a json string, just wipe sensitive word.')
    ...
    def _process_json(cls, json_data: dict, sensitive_word_list: list):
        ...
        try:
            ...
            logger.error(f'Not supported type, e={str(e)}')
            return json_data
    ...
    def _process_list(cls, list_data: list, sensitive_word_list: list):
        try:
            for i, v in enumerate(list_data):
                list_data[i] = cls._process_data(v, sensitive_word_list)
            return list_data
        except Exception as e:
            logger.error(f'Not supported type, e={str(e)}')
            return list_data
...
def anonymize(var):
    """
    将传入的参数转换成 参数名:参数值 的形式，如果参数名包含敏感信息，则将值进行匿名化处理
    :param var: 需要匿名化处理的参数
    :return: 匿名化后的 key:value 形式的字符串
    """
    traceback = inspect.getframeinfo(inspect.currentframe().f_back)  # 获取调用者的堆栈帧信息
    for line in traceback.code_context:
        m = re.search(r'\banonymize\s*\(\s*([A-Za-z_][A-Za-z0-9_.]*)\s*\)', line)
        if not m:
            continue

        var_name = m.group(1)  # 获取匹配到到参数名
        result, sens_word = Anonymity.is_sensitive_word(var_name, Anonymity.sensitive_words)
        if result:
            return f'{var_name}:******'
        else:
