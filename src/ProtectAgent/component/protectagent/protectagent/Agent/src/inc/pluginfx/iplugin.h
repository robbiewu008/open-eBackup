 ...
 *
 * @file iplugin.h
 * @brief  The implemention about iplugin.h
 * @version 1.0.0.0
 * @date 2020-08-01
...
public:
    typedef struct {
        ...
        mp_string ClassName;        // 类名字
    } DYN_CLASS;

public:
    // 插件初始化方法
    // param [in]  pMgr  PluginManager对象指针
    virtual mp_int32 Initialize(std::vector<mp_uint32> &cmds) = 0;  // 初始化方法
    ...
    virtual mp_void SetOption(mp_string pszName, mp_string pvVal) = 0;
    ...
    // retval true  参数有效
    // retval false 不支持该参数
    virtual mp_bool GetOption(mp_string pszName, mp_string& pvVal) = 0;

    // 根据输入类名创建一个对象（组件 or 具体实现）
    // [in]  pszName  对象类名
    // retval  !=NULL 成功创建的对象的指针
    // retval  NULL   创建对象失败
    virtual mp_void* CreateObject(mp_string pszName) = 0;
    ...
    // 获取加载插件的名称
    virtual mp_string GetName() = 0;

    // 获取加载插件的版本
    virtual mp_string GetVersion() = 0;

    // 获取加载插件的SCN
    virtual std::size_t GetSCN() = 0;

    // 获取插件类型
    virtual mp_int32 GetTypeId() const
    {
        return 0;
    }
...
public:
    ...
    // retval NULL   没有版本信息
    // retval !=NULL 版本信息
    virtual mp_string GetReleaseVersion(const mp_string& pszLib, mp_string& pszVer) = 0;
...
public:
    ...
    // retval != NULL  返回插件接口
    // retval NULL     插件还没有加载
    virtual IPlugin* GetPlugin(mp_string pszPlg) = 0;

    virtual IPlugin* LoadPlugin(mp_string pszPlg) = 0;

    virtual mp_void UnloadPlugin(mp_string pszPlg) = 0;
