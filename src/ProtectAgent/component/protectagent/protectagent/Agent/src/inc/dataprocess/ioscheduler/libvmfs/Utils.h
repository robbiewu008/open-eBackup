...
const int NUMBER_1024 = 1024;
const int M_SECTOR_SIZE = 512;
const int M_BLK_SIZE = 4096;
...
#define ALIGN_NUMBER(val, mult) (((val) + ((mult)-1)) & ~(((mult)-1)))
#define ALIGN_PTR(ptr, mult) (void *)ALIGN_NUMBER((uintptr_t)(ptr), mult)
...
#define DECL_ALIGNED_BUFF_WOL(name, size)  \
    u_char __##name[(size) + M_SECTOR_SIZE]; \
    u_char *(name) = (u_char *)ALIGN_PTR(__##name, M_SECTOR_SIZE)
