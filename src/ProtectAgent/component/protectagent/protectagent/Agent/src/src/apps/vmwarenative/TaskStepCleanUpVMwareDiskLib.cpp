 ...
 *
 * @file TaskStepCleanUpVMwareDiskLib.cpp
 * @brief  The implemention about TaskStepCleanUpVMwareDiskLib
 * @version 1.0.0.0
 * @date 2020-08-01
...
TaskStepCleanUpVMwareDiskLib::TaskStepCleanUpVMwareDiskLib(
...
mp_int32 TaskStepCleanUpVMwareDiskLib::Init(const Json::Value &param)
{
    ...
    if (CalibrationFormatString(strVMVer) == MP_FAILED) {
        return MP_FAILED;
    }
    GET_JSON_STRING(vmPM, PARAM_KEY_PRODUCTMANAGER_IP, strVMIp);
    if (CheckIpAddressValid(strVMIp) == MP_FAILED) {
        return MP_FAILED;
    }
...
mp_int32 TaskStepCleanUpVMwareDiskLib::Run()
{
    mp_int32 iRet = DataProcessLogic(m_reqMsgToDataProcess,
        m_respMsgFromDataProcess,
        EXT_CMD_VMWARENATIVE_CLEANUP_VDDKLIB,
        EXT_CMD_VMWARENATIVE_CLEANUP_VDDKLIB_ACK);
    if (iRet != MP_SUCCESS) {
        ERRLOG("Data process logic exec failure, task id '%s'.", m_taskId.c_str());
    }

    // Note: here should to remove parent task related attrs from cache
