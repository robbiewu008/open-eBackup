...
TaskStepCheckDBOpen::TaskStepCheckDBOpen(
...
mp_int32 TaskStepCheckDBOpen::Init(const Json::Value& param)
{
    LOGGUARD("");
...
mp_int32 TaskStepCheckDBOpen::Run()
{
    COMMLOG(OS_LOG_INFO, "Begin to check database %s open.", dbName.c_str());
    ...
    if (iRet != MP_SUCCESS) {
        COMMLOG(
            OS_LOG_ERROR, "check database %s open failed, ret %d.", dbName.c_str(), iRet);
        return iRet;
    }
#endif

    COMMLOG(OS_LOG_INFO, "taskid %s, check database %s open succ.", m_taskId.c_str(), dbName.c_str());
    return MP_SUCCESS;
}

mp_void TaskStepCheckDBOpen::BuildCheckDBScriptParam(mp_string& strParam)
{
    strParam = ORACLE_SCRIPTPARAM_INSTNAME + instName + NODE_COLON +
        ...
        ORACLE_SCIPRTPARAM_CHECKTYPE + "1";
    ClearString(dbPwd);
}

mp_int32 TaskStepCheckDBOpen::Stop(const Json::Value& param)
{
    return MP_SUCCESS;
}

mp_int32 TaskStepCheckDBOpen::Cancel()
{
    return MP_SUCCESS;
}
