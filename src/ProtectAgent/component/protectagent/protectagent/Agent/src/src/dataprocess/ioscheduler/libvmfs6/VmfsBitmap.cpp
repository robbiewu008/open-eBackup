...
int VmfsBitmap::VmfsBMERead(VmfsBitmapEntryT *bme, const u_char *buf, int copy_bitmap)
{
    ...
    bme->id = ReadLE32(buf, VMFS6_BME_OFFSET_ID);
    bme->total = ReadLE32(buf, VMFS6_BME_OFFSET_TOTAL);
    bme->free = ReadLE32(buf, VMFS6_BME_OFFSET_FREE);
    bme->ffree = ReadLE32(buf, VMFS6_BME_OFFSET_FFREE);

    if (copy_bitmap) {
        int ret = memcpy_s(bme->bitmap, (bme->total + NUM_SEVEN) / NUM_EIGHT, &buf[VMFS6_BME_OFFSET_BITMAP],
...
off_t VmfsBitmap::VmfsBitmapGetItemPos(VmfsBitmapT *b, uint32_t entry, uint32_t item)
{
    ...
    pos += b->bmh.bmpEntriesPerArea * VMFS6_BITMAP_ENTRY_SIZE;
...
{
    COMMLOG(OS_LOG_DEBUG, "");
    u_char buf[VMFS6_BITMAP_ENTRY_SIZE];
...
{
    COMMLOG(OS_LOG_DEBUG, "");
    V6_DECL_ALIGNED_BUFFER(buf, VMFS6_BITMAP_ENTRY_SIZE);
    VmfsBitmapEntryT entry;
    off_t pos;
    uint32_t addr;
    u_int array_idx;
