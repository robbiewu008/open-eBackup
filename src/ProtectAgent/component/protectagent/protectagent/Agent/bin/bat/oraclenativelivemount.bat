...
set LOGFILE=oraclenativelivemount.log
...
for /f %%a in ('type %CURRENTPIDRST%') do (
    if !NUM! NEQ 0 (
        ...
        if "!BLACKFLAG!" == "1" (
            more %CURRENTCMDLineRST% | findstr /c:"oraclenativelivemount.bat\" \"%AGENT_ROOT%\" %PID%"
        ) else (
            more %CURRENTCMDLineRST% | findstr /c:"oraclenativelivemount.bat %AGENT_ROOT% %PID%"
        )
        if !errorlevel! EQU 0 (
...
call :Log "DBNAME=%DBNAME%;DBUSER=%DBUSER%;DBINSTANCE=%DBINSTANCE%;CURRENTPID=%CURRENTPID%;AUTHMODE=!AUTHMODE!;ASMSIDNAME=!ASMSIDNAME!;ASMUSER=!ASMUSER!;DATAPATH=!DATAPATH!;LOGPATH=!LOGPATH!;METADATAPATH=!METADATAPATH!;RECOVERTARGET=!RECOVERTARGET!;RECOVERPATH=!RECOVERPATH!;RECOVERORDER=!RECOVERORDER!;ADDITIONAL=!ADDITIONAL!"
...
rem get livemount head pit scn and pit time
set maxScn=
...
if "!serviceStatus!" EQU "2" (
    set ERR_CODE=1 
    call :Log "DatabaseService !DBINSTANCE! is open, can't livemount to host."
    goto :error
)

if exist "!ADDITIONAL!\livemount" (
    call :StartLiveMountDB
    call :Log "start instance !DBINSTANCE! successfully."
) else (
    ...
    call :ExecDBRestore
    call :Log "livemount instance !DBINSTANCE! successfully."
)
...
:ModifyPfile
    ...
    set "ctlFile=!DATAPATH!\CONTROLFILE.CTL"
    set tmpfile="%AGENT_TMP_PATH%\replacetmp.txt%PID%"
    call :DeleteFile !tmpfile!
...
:ExecDBRestore
    call :Log "Begin to exec DB !DBNAME! livemount"
    set DBID=
    set DBUName=
    set DBInst=
    set ASMUser=
    ...
    set /a OMF_ENABLE=0
    rem construct log file, using OMF, set DB_CREATE_FILE_DEST, others, rename log file
    for /f "tokens=1,2 delims=; " %%a in ('type !LOG_FILES!') do (
        if "!OMF_ENABLE!" EQU "0" (
            ...
            if "!dirFst!" == "O1_MF_" (
                ...
                echo    ALTER DATABASE RENAME FILE '!logPath!' TO '!DATAPATH!!FileSep!!fileName!'; >> %EXECSQL%
            )
        )
    )
    
    if "!OMF_ENABLE!" NEQ "0" ( 
        echo    ALTER SYSTEM SET DB_CREATE_FILE_DEST='!DATAPATH!' SCOPE=BOTH; >> %EXECSQL%
        echo    ALTER SYSTEM SET DB_CREATE_ONLINE_LOG_DEST_1='!DATAPATH!' SCOPE=BOTH; >> %EXECSQL%
        echo    ALTER SYSTEM SET DB_CREATE_ONLINE_LOG_DEST_2='' SCOPE=BOTH; >> %EXECSQL%
    )
    
    echo    SET UNTIL SCN !maxScn!; >> %EXECSQL%
    ...
    call :Log "Begin to livemount with copy database !DBINSTANCE!"
    call %COMMONFUNC% "%AGENT_ROOT%" execrmansql %PID% %LOGFILE% %EXECSQLRMAN% %EXECSQLRMANRST% %DBINSTANCE% "!RMAN_LOGIN!" 600 RetCode
    if "!RetCode!" EQU "0" (
        call :Log "livemount with copy database !DBINSTANCE! successfully."
    ) else (
        call :Log "livemount with copy database !DBINSTANCE! failed, ret !RetCode!."
    ...
    if "!isCDB!" == "1" (
        ...
        if "!RetCode!" NEQ "0" (
            call :Log "[WAR]livemount:open pluginDB !DBINSTANCE! failed, ret !RetCode!."
            type %EXECSQLRST% >> %LOGFILEPATH%
        )
    )
    rem make restore completed
    echo. > "!ADDITIONAL!\livemount"
goto :eof

:StartLiveMountDB
    call :Log "Begin to start DB !DBNAME! directly"
    set DBID=
    for /f "tokens=1 delims=; " %%a in ('type "!ADDITIONAL!\dbinfo"') do (
        set DBID=%%a
        set DBID=!DBID:~1!
    )
    ...
    echo alter database open; >> %EXECSQL%
    echo EXIT >> %EXECSQL%
    ...
    call %COMMONFUNC% "%AGENT_ROOT%" execrmansql %PID% %LOGFILE% %EXECSQLRMAN% %EXECSQLRMANRST% %DBINSTANCE% "!RMAN_LOGIN!" 600 RetCode
    if "!RetCode!" EQU "0" (
        call :Log "livemount database !DBINSTANCE! successfully."
    ) else (
        call :Log "livemount database !DBINSTANCE! failed, ret !RetCode!."
        set ERR_CODE=!RetCode!
        goto :error
    )
goto :eof
...
:CreateSpfile
    call :Log "Begin to create DB !DBNAME! spfile"
    call :StopInstance
    ...
    if "!RetCode!" NEQ "0" (
        call :Log "[ERR]livemount:create database !DBINSTANCE! spfile failed, ret !RetCode!."
        type %EXECSQLRST% >> %LOGFILEPATH%
        set ERR_CODE=!RetCode!
        goto :error
    )
