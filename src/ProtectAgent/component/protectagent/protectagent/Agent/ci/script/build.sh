...
if [ "${systemtypeage}" != "ASAN" ];then
    sh ${WORKSPACE}/Agent/build/download_opensrc.sh
    cd ${WORKSPACE}/
    ...
    if [[ ${packagetype} == 'sdk' ]] ;then
        ...
        cp ${WORKSPACE}/Agent/plugin_sdk.tar.gz ${WORKSPACE}/tmp/
        artget push -p "{'branch':'${branch}','systemtypeage':'${systemtypeage}' }" -d ${WORKSPACE}/Agent/ci/LCRP/conf/sdk_from_cmc.xml -ap ${WORKSPACE}/tmp -user ${cmc_user} -pwd ${cmc_pwd}
    else
        if [ "$1" == "sanclient" ];then
        ...
        artget push -p "{'branch':'${branch}' }" -d ${WORKSPACE}/Agent/ci/LCRP/conf/pkg_from_cmc_linux.xml -ap ${WORKSPACE}/AGENT_PACK_TEMP/*.tar.xz -user ${cmc_user} -pwd ${cmc_pwd}
        echo "Build done."
    fi
else
    cd ${WORKSPACE}
