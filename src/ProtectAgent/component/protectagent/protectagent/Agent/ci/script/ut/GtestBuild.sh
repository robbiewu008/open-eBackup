...
##libttool update,automake need export echo 
export echo=echo 
...
HOME_TEST_DIR=${HOME_AGENT}/test/stubTest
...
createLcovReport()
{
    ...
    rm -rf ${HOME_AGENT}/test/stubTest/html
    ...
    sh ${HOME_AGENT}/test/stubTest/bin/run.sh >${OUTPUT_RUN_LOG} 2>${OUTPUT_RUN_ERR}
    chmod +x ${HOME_AGENT}/test/stubTest/bin
    #recover ${HOME}
    HOME=${HOME_BAK}

    mkdir -p ${HOME_AGENT}/test/stubTest/temp
    # generate
    lcov -d ${HOME_AGENT} -o ${HOME_AGENT}/test/stubTest/temp/totalAll.info -b ${HOME_AGENT} -c
    # delete open src file
    lcov --remove ${HOME_AGENT}/test/stubTest/temp/totalAll.info '/usr/include/*' "${HOME_AGENT}/open_src/*" "${HOME_AGENT}/platform/*" -o ${HOME_AGENT}/test/stubTest/temp/total.info
    genhtml -o ${HOME_AGENT}/test/stubTest/html ${HOME_AGENT}/test/stubTest/temp/total.info
    gensummary ${HOME_AGENT}/test/stubTest/temp/total.info ${HOME_AGENT}/test/stubTest/summary.xml
    rm -rf ${HOME_AGENT}/test/stubTest/temp/
    find ${HOME_AGENT}/test/stubTest/obj -name "*.gcno" | xargs rm -f
    find ${HOME_AGENT}/test/stubTest/obj -name "*.gcda" | xargs rm -f
    cd - >/dev/null
...
createGtestReport()
{
	...
	#write failed UT project to file.
    grep "<failure " ${OUTPUT_GTEST_HOME}/* | awk -F ".xml" '{print $1".xml"}' | awk -F"/" '{print $NF}' | sort -u > ${OUTPUT_GTEST_HOME}/report_failure.xml
