...
typedef enum {
    BSA_CopyType_ANY = 1, // Used for matching any copy type (for example, "backup" or "archive" in the copy type field of structures for selecting query results).
...
typedef enum {
    ...
    BSA_ObjectStatus_NOT_MOST_RECENT = 3 // Indicates that this is not the most recent backup copy, or that the object itself no longer exists.
} BSA_ObjectStatus;

// describes the original data type of the object
 typedef enum {
    BSA_ObjectType_ANY = 1, // Used for matching any object type (for example, "file" or directory") value in the object type field of structures for selecting query results.
    BSA_ObjectType_FILE = 2, // Used by the application to indicate that the type of application object is a "file" or single object.
    BSA_ObjectType_DIRECTORY = 3, // Used by the application to indicate that the type of application object is a "directory" or container of objects.
    BASBSA_ObjectType_OTHER = 4 // Used by the application to indicate that the type of application object is neither a "file" nor a "directory".
} BSA_ObjectType;
...
// and should not be modified by the XBSA Application. The XBSA Application should only write to the data portion of the buffer, 
// which is the only portion used for transferring application data.
...
typedef struct {
    ...
    BSA_UInt32  numBytes; // Actual number of bytes read from or written to the buffer, or the minimum number of bytes needed
    BSA_UInt32  headerBytes; // Number of bytes used at start of buffer for header information (offset to data portion of buffer)
...
//
// The concept of an Object space is used to provide a primary grouping of XBSA Objects, which may be used for object search 
// by a user and/or for object management by the Backup Service. Additional groupings are provided by Filespec and by object attributes. 
...
// The mandatory objectName must have a non-empty string in the pathName field. The mandatory createTime must be a valid time in UTC. 
...
typedef struct {
    ...
    BSA_UInt64          restoreOrder; //Provides hints to the XBSA Application that allow it to optimize the order of object retrieval requests //service,o
    char                rsv2[31];
    char                rsv3[31];
...
XBSA_EXPORT_API int BSACreateObject(long bsaHandle, BSA_ObjectDescriptor *objectDescriptorPtr, BSA_DataBlock32 *dataBlockPtr);
...
XBSA_EXPORT_API int BSAGetObject(long bsaHandle, BSA_ObjectDescriptor *objectDescriptorPtr, BSA_DataBlock32 *dataBlockPtr);

XBSA_EXPORT_API int BSAInit(long *bsaHandlePtr, BSA_SecurityToken *tokenPtr, BSA_ObjectOwner *objectOwnerPtr, char **environmentPtr);

XBSA_EXPORT_API int BSAQueryApiVersion(BSA_ApiVersion *apiVersionPtr);

XBSA_EXPORT_API int BSAQueryObject(long bsaHandle, BSA_QueryDescriptor *queryDescriptorPtr, BSA_ObjectDescriptor *objectDescriptorPtr);
