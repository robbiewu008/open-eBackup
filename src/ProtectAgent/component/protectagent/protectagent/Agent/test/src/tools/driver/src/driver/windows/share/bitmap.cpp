...
HBitmap *hbitmap_alloc(uint64_t size, int granularity, PBITMAP_ALLOC_FUNCTION alloc_fun, PBITMAP_FREE_FUNCTION free_fun, uint32_t tag)
{
	...
	hb = (HBitmap *)alloc_fun(sizeof(struct HBitmap), tag);
    if (NULL == hb)
    {
        ///XXXIOMIRXXXROR_ERROR("malloc for hb failed.");
        goto fail;
    }
	memset(hb, 0, sizeof(HBitmap));
    ...
    for (i = HBITMAP_LEVELS; i-- > 0; )
    {
		...
		hb->levels[i] = (unsigned long *)alloc_fun(size*sizeof(unsigned long), tag);
        if (NULL == hb->levels[i])
        {
            ////XXXIOMIXXXRROR_ERROR("malloc for hb->levels failed.");
            goto fail;
        }
