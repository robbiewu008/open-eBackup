...
4.  下载[open-eBackup-bin.tgz](https://support.huawei.com/carrier/navi?lang=en&coltype=software#col=software&detailId=PBI1-264151826&path=PBI1-262732232/PBI1-262732236/PBI1-261607562/PBI1-251366310/PBI1-258115661)到当前目录并解压。
...
1.  给节点添加标签
    ...
    ```
#### 配置cri-docker<a name="ZH-CN_TOPIC_0000002064194990"></a>
  1.下载cni-plugins-linux-arm64-v0.8.6.tgz 和cri-dockerd-0.3.16.arm64.tgz。
  2.解压cni-plugins-linux-arm64-v0.8.6.tgz文件，并且放入 /opt/cni/bin/
   ```
  tar -xzvf cni-plugins-linux-arm64-v0.8.6.tgz -C /opt/cni/bin/
    ```
  3.解压cri-dockerd-0.3.16.arm64.tgz 文件，并且放入/usr/local/bin
     ```
  tar -xzvf cri-dockerd-0.3.16.arm64.tgz
  cp cp cri-dockerd/cri-dockerd /usr/local/bin/
     ```
 如果这里不加上述配置，默认用kubeadm初始化集群后，kubelet将无法获取网络，原因是K8S应该是在1.24版本后，使用cir-docker，而且本文后面建议集群使用1.31.0
  即使你成功将flannel 网络部署后，通过kubectl get node 也会显示node 处于not reday状态。
...
备注：默认用户名sysadmin 密码Admin@123，默认端口可以通过kubectl describe pod -n dpa protectmanager-biz-service-0  |grep  server.port:查询
